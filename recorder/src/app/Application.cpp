...
void Application::encoder_loop(std::stop_token token) {
  ...
  while (!token.stop_requested()) {
    ...
    try {
      ...
      last_encoded_frame_tick_ = GetTickCount64();
...
bool Application::capture_ready() const {
  const auto tick = last_encoded_frame_tick_.load();
  return capture_->state() == CaptureState::Running && tick && GetTickCount64() - tick < 3000 && stream_description().width != 0;
}
...
nlohmann::json Application::handle_command(const nlohmann::json& request) {
  ...
  if (command == "record_start") { auto desc = stream_description(); if (!capture_ready()) return {{"success", false}, {"error", "CAPTURE_NOT_READY"}, {"message", "Game capture is still starting"}}; const bool ok = recording_->start(parse_metadata(request), desc); if (ok) encoder_->request_keyframe(); return {{"success", ok}, {"message", ok ? "Recording start requested" : "Recording already active"}}; }
  if (command == "record_stop") {
    ...
    bool broker_enabled{}, broker_configured{}; { std::scoped_lock lock(settings_mutex_); broker_enabled = config_.broker.enabled; broker_configured = !config_.broker.endpoint.empty() && BrokerUploader::build_configured(); }
    if (!path.empty() && upload_requested && broker_enabled && broker_configured && broker_) {
    ...
    } else { response["upload_queued"] = false; if (upload_requested) response["upload_error"] = !broker_enabled ? "YOUTUBE_DISABLED" : "YOUTUBE_NOT_CONFIGURED"; }
  ...
  if (command == "settings_set") {
    ...
    return {{"success", true}, {"message", "Settings saved"}, {"youtube_enabled", config_.broker.enabled}, {"youtube_configured", !config_.broker.endpoint.empty() && BrokerUploader::build_configured()}, {"audio_enabled", config_.audio.enabled}, {"microphone_enabled", config_.audio.microphone}, {"microphone_supported", true}};
  }
  ...
  if (command == "replay_save") {
    if (replay_->state() != ReplayState::Running) return {{"success", false}, {"error", "REPLAY_NOT_RUNNING"}, {"message", "Replay buffer is not running"}}; auto desc = stream_description(); if (!capture_ready()) return {{"success", false}, {"error", "CAPTURE_NOT_READY"}, {"message", "No recent encoded video is available"}};
  ...
  if (command == "status") {
    nlohmann::json j{{"success", true}, {"capture", capture_ready()}, {"capture_state", std::string(to_string(capture_->state()))}, {"game_found", capture_->hwnd() != nullptr}, {"recording", recording_->state() == RecordingState::Recording || recording_->state() == RecordingState::WaitingForKeyframe}, {"recording_state", std::string(to_string(recording_->state()))}, {"recording_seconds", recording_->duration_hns() / HnsPerSecond}, {"replay_buffer", replay_->state() != ReplayState::Stopped && replay_->state() != ReplayState::Error}, {"replay_state", std::string(to_string(replay_->state()))}, {"replay_seconds", config_.replay.seconds}, {"replay_available_seconds", replay_->available_hns() / HnsPerSecond}, {"encoder", encoder_->encoder_name()}, {"fps", config_.video.fps}, {"dropped_frames", dropped_frames_.load()}, {"save_id", save_generation_}};
    const auto recording_directory = config_.recording.directory.u8string();
    j["recording_directory"] = std::string(recording_directory.begin(), recording_directory.end());
    auto d = stream_description(); j["width"] = d.width; j["height"] = d.height; j["audio"] = audio_->running();
    { std::scoped_lock lock(settings_mutex_); j["youtube_enabled"] = config_.broker.enabled; j["youtube_configured"] = !config_.broker.endpoint.empty() && BrokerUploader::build_configured(); j["youtube_channel"] = config_.broker.channel_title; j["audio_enabled"] = config_.audio.enabled; j["microphone_enabled"] = config_.audio.microphone; j["microphone_supported"] = true; }
    { auto upload = upload_queue_->status(); j["youtube_upload_in_progress"] = upload.value("in_progress", false); j["youtube_upload_phase"] = upload.value("phase", ""); j["youtube_upload_sent_bytes"] = upload.value("sent_bytes", 0ull); j["youtube_upload_total_bytes"] = upload.value("total_bytes", 0ull); j["youtube_upload_percent"] = upload.value("percent", 0u); j["youtube_upload_pending"] = upload.value("pending", 0u); j["youtube_last_upload_success"] = upload.value("last_success", false); j["youtube_last_upload_url"] = upload.value("last_url", ""); j["youtube_last_upload_error"] = upload.value("last_error", ""); j["youtube_last_upload_target_id"] = upload.value("last_target_id", -1); j["youtube_last_upload_target_name"] = upload.value("last_target_name", ""); }
